...
- Groq decommissions models with little or no notice — llama-4-scout vanished 18.07.2026 (no notice), llama-3.3-70b-versatile went 16.08.2026 (email 2 days ahead) and `qwen/qwen3-32b` silently disappeared around July. When a model 404s, check the live list (`curl -H "Authorization: Bearer $GROQ_API_KEY" https://api.groq.com/openai/v1/models`), and ALWAYS measure real limits from the `x-ratelimit-limit-*` headers of a live request, not docs.
- Groq **reasoning models** (gpt-oss, qwen) need `reasoning: true` in `MODELS` so `providers.ts` sends `reasoning_format: "hidden"` — otherwise their `<think>…</think>` deliberation streams into the UI. Do NOT flag non-reasoning models (incl. `groq/compound-mini`) — Groq 400s on the param there. Compound models put reasoning in a separate response field we never read.
