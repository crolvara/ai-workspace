...
- **OCR**: Tesseract.js v7, fully client-side (`src/components/ocr/ocr-tool.tsx`, dynamic import, `bul`+`eng`); worker/core/langdata load from the jsdelivr CDN pinned to the installed version — keep the npm package and CDN in sync when upgrading.
  - **"Clean up with AI"** (added 2026-07-17) is the one part of OCR that leaves the browser: recognition stays local, but this optional button posts the raw text to `/api/chat` with `ephemeral: true` to strip OCR noise / restore structure. It reuses the chat endpoint (rate-limit → session → `UsageLog`, SSE via `readChatStream`) instead of a new route — `/api/chat` has **no system-prompt field**, so the cleanup instructions are embedded in the user message (`buildStructurePrompt`); model is `DEFAULT_MODEL_KEY`. Input is capped at 7000 chars client-side (chat route rejects a message >8000). Invariant: the textarea is overwritten live while streaming, so on any failure (mid-stream `error` event, network throw, or empty/whitespace result) it restores the saved `original` text — never leave the user with half-cleaned output and no original. New OCR is locked (`inputLocked = isBusy || isStructuring`) during a cleanup stream or the two `setText` loops race.
